...
class SqueezeExcitation(nn.Module):
    ...
    def forward(self, x):  # 1 ， 3， 224 ，224
        x_se = F.adaptive_avg_pool2d(x, 1)  # 1， 3， 1， 1
        x_se = self.reduce(x_se)  # 1 ， 1， 1， ，1
        x_se = self.act_fn(x_se)
        x_se = self.expand(x_se)  # 1  ， 3， 1， 1
        x = x * x_se.sigmoid()  # 1， 3， 224， 224
...
class MBConv(nn.Module):
    def __init__(self, in_channels, out_channels,
        ...
        if expand_ratio != 1:
            self.expand_conv = Conv2dBlock(in_channels, expand_channels, 1, bias=False, momentum=momentum, eps=eps,
                                           act_fn=self.act_fn, static_padding=True, image_size=image_size)
        self.depthwise_conv = DepthwiseConv2dBlock(expand_channels, ksize, stride=stride, momentum=momentum, eps=eps,
                                                   act_fn=self.act_fn, static_padding=True, image_size=image_size)
        image_size = np.ceil(image_size / stride).astype(int)
        if se_ratio is not None:
            self.se = SqueezeExcitation(expand_channels, se_ratio=se_ratio, act_fn=self.act_fn, static_padding=True,
                                        image_size=image_size)
        self.project_conv = Conv2dBlock(expand_channels, out_channels, 1, bias=False, momentum=momentum, eps=eps,
                                        static_padding=True, image_size=image_size)
...
if __name__ == '__main__':
    ...
    model = MBConv(96, 96, expand_ratio=4, image_size=[112, 112], stride=2)
    output = model(x)
    print(output)
