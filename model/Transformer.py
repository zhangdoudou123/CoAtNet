...
class RelativeAttention(nn.Module):
    ...
    def forward(self, x):  # b c h w
        ...
        # 1 384 14 14 32 --s3
        len_x = H * W  # 196
        x = x.view(b, c, len_x).transpose(-1, -2)  # 1 196 384
        q = self.Q(x).view(b, len_x, self.n_head, self.d_k).transpose(1, 2)  # 1  196 32 384/32 -- 1  32 196 12
        k = self.K(x).view(b, len_x, self.n_head, self.d_k).transpose(1, 2)  # 1  32 196 12
        v = self.V(x).view(b, len_x, self.n_head, self.d_v).transpose(1, 2)  # 1  32 196 12
        ...
        relative_biases = relative_bias.gather(dim=-1, index=relative_indices)  # [196  196]

        similarity = torch.matmul(q, k.transpose(-1, -2)) + relative_biases  # [196  196]
        similarity = similarity.softmax(dim=-1)  # 1  32 196 196
        similarity = self.attn_dropout(similarity)

        out = torch.matmul(similarity, v)  # 【196 196】 [196 12 ] --1 32  196 12
        out = out.transpose(1, 2).contiguous().view(b, -1, self.n_head * self.d_v)  # 1 196 384
        # 1 196 32 12 -- 1  196   384
        out = self.ff(out)  # 1  196   384
        out = self.ff_dropout(out)
        out = out.transpose(-1, -2).view(b, -1, H, W)  # 1  384 196 -- 1 384 14 14
        return out
...
class FeedForwardRelativeAttention(nn.Module):
    ...
    def forward(self, x):  # 1 384 14 14
        x_in = x
        x = self.fc1(x)  # 1 1416 14 14
        x = self.act_fn(x)
        x = self.fc2(x)  # 1 384 14 14
        x = drop_connect(x, drop_ratio=self.drop_ratio, training=self.training) + x_in
        return x  # 1 384 14 14
...
class TransformerWithRelativeAttention(nn.Module):
    ...
    def forward(self, x):  # n c h w
        ...
        x = self.attention(x)  # n c h/2 w/2
        x = drop_connect(x, self.dropout, training=self.training)
        x = x_stem + x  # n c h/2 w/2
        x_attn = x
        x = self.ff(x)  # n c h/2 w/2
        x = drop_connect(x, self.dropout, training=self.training)
        x = x_attn + x  # n c h/2 w/2
        return x  # n c h/2 w/2


if __name__ == '__main__':
    x = torch.rand(1, 384, 14, 14)  # s3
    model = TransformerWithRelativeAttention(14, 14, 384, 32)
    output = model(x)
    print(output)
